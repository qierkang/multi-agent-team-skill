...
def migrate_threads(items: list[dict[str, Any]], policy: dict[str, Any]) -> list[dict[str, Any]]:
    status_map = {
    ...
    for index, item in enumerate(items, 1):
        ...
        if not isinstance(evidence, list) or not all(isinstance(path, str) for path in evidence):
            raise InstallError(f"v1 任务 {thread_id} evidence_paths 非法")
...
def validate_migrated_threads(threads: list[dict[str, Any]], policy: dict[str, Any]) -> None:
    ...
    if len(active) > int(policy["max_active_long_threads"]):
...
def reconfigure_v2_models(
    ...
    *,
    ...
    if not model_overrides and not version_sync:
    ...
    updated_state.update({"model_tiers": new_tiers, "skill_version": SKILL_VERSION, "updated_at": now})
    ...
    updated_snapshot.update({"threads": remapped_threads, "updated_at": now})
    ...
    event = "model_tiers_reconfigured" if model_overrides else "skill_patch_upgraded"
    events.append({"at": now, "event": event, "details": {"from": old_tiers, "to": new_tiers}})
    ...
    print(f"REMAPPED_THREADS={len(remapped_threads)}")
    ...
    if not apply:
        ...
        print("STATE=model_reconfiguration_plan_ready" if model_overrides else "STATE=patch_upgrade_plan_ready")
        return 0
    ...
    print("STATE=models_reconfigured" if model_overrides else "STATE=team_patch_upgraded")
...
def main() -> int:
    ...
    parser.add_argument("--thread-mode", choices=["recommend", "controlled-auto"], default="recommend")
    ...
    try:
        ...
        state_payloads = state_defaults(args.thread_mode, model_tiers)
        registry = state_payloads[THREAD_REGISTRY]
        registry["threads"] = migrate_threads(legacy_threads, state_payloads[PROJECT_STATE])
        ...
        print(f"THREAD_MODE={args.thread_mode}")
        for tier in ("fast", "standard", "advanced"):
